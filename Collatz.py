...
def collatz_eval(i, j):
    ...
    # Assertions to Verify Valid Input Range
    if i <= 0:
        raise ValueError("All inputs must be greater than 0.")
    if j >= 1000000:
        raise ValueError("All inputs must be less than 1,000,000.")

    # Initialize a List to Store Maximum Cycle Lengths
    cycleLengths = []

    # Utilize the Thousands Eager Cache if the given range has 2000 values or more
    start = i//1000
    end = j//1000

    # If the range is 2000 or more...
    if end - start > 1:

        # Individually process values from the starting point up until the next thousand (inclusive)
        for num in range(i, ((start+1)*1000)+1):
            count = 1
            while num != 1:
                if num % 2 == 0:
                    num = num/2
                    ...
                    num = 3*num + 1
                    count += 1
            cycleLengths.append(count)

        # Then, append the highest MCL between that next thousand and the largest multiple of a thousand in the range
        cycleLengths.append(max(ThousandsEagerCache[(start+1): end]))

        # Finally, individually append any remaining MCL values above that highest multiple
        for num in range((end*1000)+1, j+1):
            count = 1
            while num != 1:
                if num % 2 == 0:
                    num = num/2
    ...
    # Otherwise, simply compute all MCLs in the given range
    else:

        for num in range(i, j+1):

            # General Logic - Keep count of the cycle length, using the "count" variable
            count = 1

            # Continuously add to count until the number becomes 1
            while num != 1:
                # If the number is even, divide it by 2
                if num % 2 == 0:
                    num = num/2
                    count += 1
                # Otherwise, the number must be odd, and we will multiply by three and add one
                else:
                    num = 3*num + 1
                    count += 1
            # When the number in the range becomes 1, append that count to the list of cycleLengths
            cycleLengths.append(count)

    assert type(max(cycleLengths)
                ) == int, "Error - Cycle Length Value is a Non-Integer."
    assert max(
        cycleLengths) > 0, "Error - Cycle Length Value is not Greater Than 0."
...
def collatz_print(w, i, j, v):
    ...
    #assert j >= i, "Please Enter a Valid Range with the Lower Number on the Left."
    #assert i > 0, "The minimum range must be greater than 0."
    #assert j < 1000000, "The maximum range must be less than 1,000,000."
    #assert type(v) == int, "Error - Cycle Length Value is a Non-Integer."
