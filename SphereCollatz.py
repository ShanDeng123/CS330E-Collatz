...
def collatz_eval(i, j):
    ...
    assert type(i) == int, "Error - Starting Range Value is a Non-Integer"
    assert type(j) == int, "Error - Ending Range Value is a Non-Integer"
    ...
    assert i > 0, "All inputs must be greater than 0."
    assert j < 1000000, "All inputs must be less than 1,000,000."
...
##    if True:
    if end - start > 1:
        ...
        Lengths.append(max(ThousandsEagerCache[(start+1): end]))
    ...
    assert type(max(Lengths)) == int, "Error - Cycle Length Value is a Non-Integer."
    assert max(Lengths) > 0, "Error - Cycle Length Value is not Greater Than 0."
    return max(Lengths)
